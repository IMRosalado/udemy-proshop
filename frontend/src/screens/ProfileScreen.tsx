...
const ProfileScreen = () => {
  ...
  const [updateProfile, { isLoading: isLoadingUpdateProfile }] = useProfileUpdateMutation();
  const {data: orders, isLoading, error: ordersError} = useGetMyOrdersQuery(null);
  ...
  return (
    <Row>
      ...
      <Col md={9}>
        <h2>My Orders</h2>
        { isLoading ? (<Loader/>): (ordersError) ? (<Message variant="danger">{'error' in ordersError ? ordersError.error : ""} </Message>) : (
